/*
  ...
  2. Stored Procedures
    - `create_server_configs_table()` - Creates the server_configs table
    - `create_verification_records_table()` - Creates the verification_records table
    
  3. Indexes
    - Index on server_configs.guild_id for faster lookups
    - Indexes on verification_records.user_id, guild_id, and token for faster lookups
    
  4. Row Level Security
    - RLS enabled on both tables for added security
*/

-- Create stored procedure for server_configs table
CREATE OR REPLACE FUNCTION create_server_configs_table()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Create stored procedure for verification_records table
CREATE OR REPLACE FUNCTION create_verification_records_table()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Create the verification_records table if it doesn't exist
